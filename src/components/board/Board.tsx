...
type BoardTile = {isLegalMove: boolean, piece: BoardPiece}
...
export const Board = () => {
    ...
    const getBoardString = () => service.send(() => ({tag: 'boardString'}))
    ...
    const getLegalMoves = () => service.send(() => ({tag: 'legalMoves'}))
            ...
            <span className={classNames.board}>
                {board.map((row, rindex) => {
                    return row.map((tile, cindex) => {
                        ...
                        const legalMoveClass = (tile.isLegalMove ? classNames["legal-move"] : "")

                        return (
                        <span className={`${legalMoveClass} ${oddnessClass}`}>
                            <Piece key={rindex + cindex} piece={tile.piece} onClick={() => {
                                for(const move of tile.piece.legalMoves) {
                                    ...
                                    setBoard(prevState => {
                                        prevState[i][j].isLegalMove = true
                                        return prevState
                                    }, true)
                                }
                            }}/>
